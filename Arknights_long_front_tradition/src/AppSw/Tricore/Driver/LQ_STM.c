...
void STM0_CH0_IRQHandler(void)
{
    ...
    //var[0]=encValue1;
	//UART_sendware(UART0, (uint8 *) var , sizeof(var));
	//UART_PutStr(UART0,"\n");
	//sprintf(txt, "Enc1: %05d; ", encValue1);
	//UART_PutStr(UART0,txt);

	if(sittu == 1 && biao == 0){
	    enconter_cnt+=(encValue1 / 11);
	    //increase_P=1;
	    increase_pwm=500;                           //两个可调参数
	    if(enconter_cnt<=-3500) {
	        //increase_P=0;
	        increase_pwm=0;
	        biao=1;
	        enconter_cnt=0;
	...
	if(biao==1 && sittu==1) {
	    LED_Ctrl(LEDALL,ON);
	    enconter_cnt+=(encValue1 / 30);
	    if(enconter_cnt<=-12500) {                    //一个可调参数
	        biao=0;
	        sittu=0;
	        enconter_cnt=0;
	    }
	...
	if(sittu_zhi == 1){
	       enconter_cnt_1+=(encValue1 / 100);

	       if(enconter_cnt_1>=-100)  {
	               zhijiao=-150;

	       }
	       else if(enconter_cnt_1<=-100 && enconter_cnt_1>=-600)  {
	                        zhijiao=0;
	                        //sprintf(txt, "qwq: %05d    %05d   \n", sittu_zhi,zhijiao);
	                        //UART_PutStr(UART0,txt);
	       }
	       else if(enconter_cnt_1<=-800) {
	                        sittu_zhi=0;
	                        enconter_cnt_1=0;
	                //LED_Ctrl(LEDALL,OFF);
	       }
...
void STM0_CH1_IRQHandler(void)
{
	...
	if(sittu_zhi == 0) {
	    ...
	    UART_PutStr(UART0,txt);
	    char time1=90;
	    sprintf(txt,"00%c%c%c%c%c%c%c%c%c",ad_data_V[0],ad_data_V[1],ad_data_V[2],ad_data_V[3],ad_data_V[4],ad_data_V[5],ad_data_V[6],aierror,time1);
	}

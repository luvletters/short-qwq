...
void UART_sendware(UART_t  uratn, unsigned char *waddr, int wsize){
    #define CMD_WARE     3

    uint8 cmdf[2] = {CMD_WARE, ~CMD_WARE};
    uint8 cmdr[2] = {~CMD_WARE, CMD_WARE};

    UART_PutBuff(uratn,cmdf,sizeof(cmdf));
    UART_PutBuff(uratn,(unsigned char *)waddr,wsize);
    UART_PutBuff(uratn,cmdr,sizeof(cmdr));
}
